...
public class VerticalTabSubLayout extends VerticalTabLayout {
    ...
    private List<VerticalTabLayout.OnTabSelectedListener> mTabSelectedListeners;
    private OnTabPageChangeListener mTabPageChangeListener;
    private DataSetObserver mPagerAdapterObserver;

    private TabFragmentManager mTabFragmentManager;

    public VerticalTabSubLayout(Context context) {
        this(context, null);
    }

    public VerticalTabSubLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public VerticalTabSubLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void setTabSelectedImpl(final int position, boolean updataIndicator, boolean callListener) {
        ...
        if (callListener) {
            for (int i = 0; i < mTabSelectedListeners.size(); i++) {
                VerticalTabLayout.OnTabSelectedListener listener = mTabSelectedListeners.get(i);
                if (listener != null) {
                    if (selected) {
...
//        mTabStrip.setIndicatorGravity();
        mTabStrip.setIndicatorGravitySub();
    }
...
//    }
    ...
    public void addOnTabSelectedListener(VerticalTabLayout.OnTabSelectedListener listener) {
        if (listener != null) {
            mTabSelectedListeners.add(listener);
        }
    }

    public void removeOnTabSelectedListener(VerticalTabLayout.OnTabSelectedListener listener) {
        if (listener != null) {
...
//            setIndicatorGravity();
        ...
        protected void setIndicatorGravitySub() {
            if (mIndicatorGravity == Gravity.LEFT) {
                mIndicatorX = 0;
                if (mLastWidth != 0) mIndicatorWidth = mLastWidth;
                setPadding(mIndicatorWidth, 0, 0, 0);
